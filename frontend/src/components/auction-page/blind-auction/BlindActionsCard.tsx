...
function BlindActionsCard({ auctionAddress, auction, refresh }: Props) {
    const [network] = useNetwork()
    const [client] = useClient()
    const [auctionContract] = useBlindAuction(auctionAddress)

    const customizedMarker = useCallback(
        (item: (typeof blindAuctionMarkers)[0]) => {
            let icon = null
            let color = null as BadgeProps['severity']
            if (auction && item.stage === BlindAuctionStage.ENDED && auction.stage === BlindAuctionStage.ENDED) {
                icon = 'pi pi-check'
                color = 'secondary'
    ...
    return (
        <Card title="Actions" className="w-full h-full">
            ...
            {auction.stage === BlindAuctionStage.ENDED && (
                <EndStage
                    auctionAddress={auctionAddress}
                    auction={auction}
                    refresh={refresh}
...
export default BlindActionsCard
